...
static void readThreadFunc(void* threadData) {
   PRINT_DEBUG("inside the readThreadFunc\n")
...
static void listenerThreadFunc(void* threadData) {
	...
	while (self->data.maxConnections > self->nConnections) {
		int slot;
		int len;
		int cpunum=1;//cpunum 1로 정해놓음.added by kkr
		/* find next free slot */
		...
		etThread_construct(
				...
				cpunum);          //soclistener thread cpunum added by kkr
...
etSocketError etStartListening(etSocketServerData* data, short port) {
	...
	int i;
	int cpunum =2;//cpunum added by kkr 일단 임으로 cpunum 정해놓음
	if (self==NULL)
		return ETSOCKET_ERROR;
	...
	etThread_construct(
			...
			listenerThreadFunc,
			self,
			cpunum);//socstartlistener added by kkr
...
etSocketError etConnectServer(etSocketConnectionData* data, const char* addr, short port) {
	etSocketConnectionDataImpl* self = (etSocketConnectionDataImpl*) data;
	struct hostent *host;
	int cpunum = 3;//cpunum added by kkr
	if (addr==NULL)
		addr = "220.67.124.131"; //IP;

	if (isalpha(addr[0])) {
		host = gethostbyname("220.67.124.131"); //IP
	}
	else {
		unsigned long a = inet_addr("220.67.124.131"); //IP
	...
	self->address.sin_port = htons(6666);
	...
	etThread_construct(
			...
			self,
			cpunum);
	//soc connect thread cpunum added by kkr
